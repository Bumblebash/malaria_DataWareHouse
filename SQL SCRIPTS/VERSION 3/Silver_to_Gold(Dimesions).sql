...
BEGIN
    ...
    BEGIN TRY
        ...
        INNER JOIN Stg_Malaria_Permanent src ON src.District = target.DistrictName
        ...
        SELECT DISTINCT src.District, r.RegionKey, CASE WHEN src.District LIKE '%CITY%' THEN 1 ELSE 0 END, GETDATE()
        ...
        UPDATE target SET target.IsCurrent = 0, target.ValidTo = GETDATE()
        FROM DimFacility target
        ...
        INSERT INTO DimFacility (Source_FacilityID, FacilityName, DistrictKey, ValidFrom, ValidTo, IsCurrent)
        SELECT DISTINCT stg.FacilityID, stg.District + ' Clinic', dist.DistrictKey, GETDATE(), NULL, 1
